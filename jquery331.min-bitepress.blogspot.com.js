!function(element, proceed) {
   ...
   * @return {undefined}
   */
  function initialize(data, context, options) {
    var key;
    var script = (context = context || doc).createElement("script");
    if (script.text = data, options) {
   ...
   */
  function type(value) {
    return null == value ? value + "" : "object" == typeof value || "function" == typeof value ? ObjProto[toString.call(value)] || "object" : typeof value;
  }
  /**
   * @param {Object} obj
   * @return {?}
  ...
  function isArraylike(obj) {
    ...
    var keys = type(obj);
    return!isArray(obj) && (!isWindow(obj) && ("array" === keys || (0 === length || "number" == typeof length && (length > 0 && length - 1 in obj))));
  }
  /**
   * @param {Node} elem
   * @param {string} name
  ...
  function winnow(elements, qualifier, not) {
    return isArray(qualifier) ? jQuery.grep(elements, function(elem, i) {
   ...
   * @return {undefined}
   */
  function require(arg, callback, fn, args) {
    var self;
    try {
      if (arg && isArray(self = arg.promise)) {
        self.call(arg).done(callback).fail(fn);
      } else {
        if (arg && isArray(self = arg.then)) {
          self.call(arg, callback, fn);
        } else {
          callback.apply(void 0, [arg].slice(args));
        }
      }
    } catch (handleEl) {
  ...
  function dataAttr(elem, udataCur, data) {
    ...
    if (void 0 === data && 1 === elem.nodeType) {
      if (ol = "data-" + udataCur.replace(r20, "-$&").toLowerCase(), "string" == typeof(data = elem.getAttribute(ol))) {
        try {
          data = render(data);
        } catch (e) {
        }
  ...
  function get(elem, prop, parts, t) {
    ...
    var start = (jQuery.cssNumber[prop] || "px" !== unit && +obj) && rfxnum.exec(jQuery.css(elem, prop));
  ...
  function update(element) {
    ...
    return display || (elem = doc.body.appendChild(doc.createElement(tag)), display = jQuery.css(elem, "display"), elem.parentNode.removeChild(elem), "none" === display && (display = "block"), _elm_display[tag] = display, display);
  }
  /**
  ...
  function manipulationTarget(elem, content) {
    return callback(elem, "table") && callback(11 !== content.nodeType ? content : content.firstChild, "tr") ? jQuery(elem).children("tbody")[0] || elem : elem;
  }
  /**
   * @param {Element} elem
   * @return {?}
            ...
            if (!data_priv.access(node, "globalEval")) {
              if (jQuery.contains(doc, node)) {
                if (node.src && "module" !== (node.type || "").toLowerCase()) {
                  ...
                  initialize(node.textContent.replace(normalizr, ""), doc, node);
                }
              }
            }
          }
  ...
  function css(elem, prop, computed) {
    ...
    return(computed = computed || getStyles(elem)) && ("" !== (val = computed.getPropertyValue(prop) || computed[prop]) || (jQuery.contains(elem.ownerDocument, elem) || (val = jQuery.style(elem, prop))), !support.pixelBoxStyles() && (regexp.test(val) && (regex.test(prop) && (width = style.width, minWidth = style.minWidth, maxWidth = style.maxWidth, style.minWidth = style.maxWidth = style.width = val, val = computed.width, style.width = width, style.minWidth = minWidth, style.maxWidth = maxWidth)))), 
    void 0 !== val ? val + "" : val;
  }
  /**
   * @param {?} $timeout
  ...
  function capitalize(s) {
    ...
    var capitalized = s[0].toUpperCase() + s.slice(1);
    /** @type {number} */
    var length = VENDOR_PREFIXES.length;
    for (;length--;) {
      if ((s = VENDOR_PREFIXES[length] + capitalized) in style) {
        return s;
      }
    }
  }
  ...
  function property(name) {
    var found = jQuery.cssProps[name];
    return found || (found = jQuery.cssProps[name] = capitalize(name) || name), found;
  }
  /**
   * @param {Object} owner
   * @param {string} value
  ...
  function cb(owner, value, actual) {
    ...
    var iterator = rfxnum.exec(value);
   ...
   * @return {?}
   */
  function fn(elem, args, extra, key, styles, count) {
    /** @type {number} */
    ...
    return!key && (count >= 0 && (val += Math.max(0, Math.ceil(elem["offset" + args[0].toUpperCase() + args.slice(1)] - count - val - baseHeight - 0.5)))), val;
  }
  /**
   * @param {Object} elem
   * @param {string} prop
  ...
  function getWidthOrHeight(elem, prop, extra) {
    ...
    /** @type {boolean} */
    var isBorderBox = "border-box" === jQuery.css(elem, "boxSizing", false, value);
    /** @type {boolean} */
    var camelKey = isBorderBox;
    if (regexp.test(val)) {
      if (!extra) {
        return val;
      }
    ...
    return camelKey = camelKey && (support.boxSizingReliable() || val === elem.style[prop]), ("auto" === val || !parseFloat(val) && "inline" === jQuery.css(elem, "display", false, value)) && (val = elem["offset" + prop[0].toUpperCase() + prop.slice(1)], camelKey = true), (val = parseFloat(val) || 0) + fn(elem, prop, extra || (isBorderBox ? "border" : "content"), camelKey, value, val) + "px";
  }
  /**
    ...
    if ((tween = !jQuery.isEmptyObject(props)) || !jQuery.isEmptyObject(orig)) {
      if (f) {
        if (1 === elem.nodeType) {
          ...
          if (null == (val = dataShow && dataShow.display)) {
            val = data_priv.get(elem, "display");
          }
          if ("none" === (type = jQuery.css(elem, "display"))) {
            if (val) {
              type = val;
            } else {
              showHide([elem], true);
  ...
  function propFilter(obj, members) {
    ...
    var hooks;
    for (key in obj) {
      if (name = trim(key), member = members[name], value = obj[key], Array.isArray(value) && (member = value[1], value = obj[key] = value[0]), key !== name && (obj[name] = value, delete obj[key]), (hooks = jQuery.cssHooks[name]) && "expand" in hooks) {
  ...
  function Animation(elem, properties, options) {
    ...
    /** @type {number} */
    var k = 0;
    var numrows = Animation.prefilters.length;
    ...
    for (;k < numrows;k++) {
      if (result = Animation.prefilters[k].call(animation, elem, props, animation.opts)) {
        return isArray(result.stop) && (jQuery._queueHooks(animation.elem, animation.opts.queue).stop = result.stop.bind(result)), result;
      }
    }
    return jQuery.map(props, createTween, animation), isArray(animation.opts.start) && animation.opts.start.call(elem, animation), animation.progress(animation.opts.progress).done(animation.opts.done, animation.opts.complete).fail(animation.opts.fail).always(animation.opts.always), jQuery.fx.timer(jQuery.extend(tick, {
      elem : elem,
      anim : animation,
  ...
  function indexOf(value) {
    return(value.match(core_rnotwhite) || []).join(" ");
  }
  /**
    ...
    return function(v, k) {
      ...
      if (isArray(k)) {
        for (;node = elem[i++];) {
          if ("+" === node[0]) {
            node = node.slice(1) || "*";
            (structure[node] = structure[node] || []).unshift(k);
          } else {
            (structure[node] = structure[node] || []).push(k);
          }
        }
      }
    };
   ...
   * @return {?}
   */
  var isArray = function isArraylike(obj) {
  ...
  jQuery.extend({
    ...
    isPlainObject : function(obj) {
      var node;
      var it;
      return!(!obj || "[object Object]" !== toString.call(obj)) && (!(node = getPrototypeOf(obj)) || "function" == typeof(it = hasOwn.call(node, "constructor") && node.constructor) && ostring.call(it) === events);
    },
    /**
     * @param {?} obj
     * @return {?}
     ...
     */
    trim : function(s) {
      return null == s ? "" : (s + "").replace(rreturn, "");
    },
    /**
     * @param {?} arr
     * @param {Array} results
  ...
  jQuery.each("Boolean Number String Function Array Date RegExp Object Error Symbol".split(" "), function(dataAndEvents, m3) {
    ObjProto["[object " + m3 + "]"] = m3.toLowerCase();
  });
  var Sizzle = function(win) {
    ...
    function addCombinator(done, element, dataAndEvents) {
      ...
      var a = dataAndEvents && "parentNode" === root;
      /** @type {number} */
      var id = rightId++;
      return element.first ? function(element, profile, code) {
        for (;element = element[parentNode];) {
          if (1 === element.nodeType || a) {
        ...
        if (code) {
          for (;element = element[parentNode];) {
            if ((1 === element.nodeType || a) && done(element, context, code)) {
              return true;
            }
          }
        } else {
     ...
     * @return {?}
     */
    function condense(collection, map, filter, context, xml) {
      var item;
      /** @type {Array} */
      var caseSensitive = [];
      /** @type {number} */
    ...
    function setMatcher(preFilter, selector, matcher, postFilter, postFinder, postSelector) {
      return postFilter && (!postFilter[expando] && (postFilter = setMatcher(postFilter))), postFinder && (!postFinder[expando] && (postFinder = setMatcher(postFinder, postSelector))), markFunction(function(seed, results, context, xml) {
        ...
        var matcherIn = !preFilter || !seed && selector ? dontCloseTags : condense(dontCloseTags, preMap, preFilter, context, xml);
        ...
        if (seed) {
          if (postFinder || preFilter) {
            ...
            for (;i--;) {
              if (elem = matcherOut[i]) {
                if ((temp = postFinder ? getDistance(seed, elem) : preMap[i]) > -1) {
                  /** @type {boolean} */
    ...
    function matcherFromTokens(tokens) {
      ...
      var matchAnyContext = addCombinator(function(walkers) {
        return getDistance(a, walkers) > -1;
      }, implicitRelative, true);
      /** @type {Array} */
      var matchers = [function(elem, context, xml) {
        var i = !leadingRelative && (xml || context !== queuedFn) || ((a = context).nodeType ? matchContext(elem, context, xml) : matchAnyContext(elem, context, xml));
        return a = null, i;
      }];
      for (;i < len;i++) {
    ...
    function matcherFromGroupMatchers(elementMatchers, setMatchers) {
      ...
      var superMatcher = function(dataAndEvents, xml, context, results, seed) {
        ...
        var setMatched = [];
        var fn = queuedFn;
        var checkSet = dataAndEvents || triggerElem && Expr.find.TAG("*", seed);
        var fx = type += null == fn ? 1 : Math.random() || 0.1;
        var l = checkSet.length;
        if (seed) {
          queuedFn = xml === doc || (xml || seed);
        }
        for (;i !== l && null != (elem = checkSet[i]);i++) {
          if (triggerElem && elem) {
            /** @type {number} */
            j = 0;
            if (!xml) {
    ...
    var jQuery = addCombinator(function(config) {
      return true === config.disabled && ("form" in config || "label" in config);
    }, {
      dir : "parentNode",
      next : "legend"
    });
    ...
    setDocument = Sizzle.setDocument = function(node) {
      ...
      return result !== doc && (9 === result.nodeType && result.documentElement) ? (doc = result, docElem = doc.documentElement, documentIsHTML = !isXML(doc), preferredDoc !== doc && ((win = doc.defaultView) && (win.top !== win && (win.addEventListener ? win.addEventListener("unload", onmessage, false) : win.attachEvent && win.attachEvent("onunload", onmessage)))), support.attributes = assert(function(div) {
        ...
        return compare || (1 & (compare = (a.ownerDocument || a) === (b.ownerDocument || b) ? a.compareDocumentPosition(b) : 1) || !support.sortDetached && b.compareDocumentPosition(a) === compare ? a === doc || a.ownerDocument === preferredDoc && contains(preferredDoc, a) ? -1 : b === doc || b.ownerDocument === preferredDoc && contains(preferredDoc, b) ? 1 : sortInput ? getDistance(sortInput, a) - getDistance(sortInput, b) : 0 : 4 & compare ? -1 : 1);
      } : function(a, b) {
        if (a === b) {
          return f = true, 0;
        }
        ...
        var bp = [b];
        if (!aup || !bup) {
          return a === doc ? -1 : b === doc ? 1 : aup ? -1 : bup ? 1 : sortInput ? getDistance(sortInput, a) - getDistance(sortInput, b) : 0;
        }
        if (aup === bup) {
    ...
    Sizzle.escape = function(attr) {
      return(attr + "").replace(rreturn, _char);
    };
    /**
     * @param {string} elem
     * @return {?}
    ...
    Sizzle.uniqueSort = function(results) {
      ...
      /** @type {number} */
      var i = 0;
      if (f = !support.detectDuplicates, sortInput = !support.sortStable && results.slice(0), results.sort(sortOrder), f) {
        for (;elem = results[i++];) {
          if (elem === results[i]) {
            /** @type {number} */
            j = duplicates.push(i);
    ...
    (Expr = Sizzle.selectors = {
      ...
      filter : {
         ...
         * @return {?}
         */
        CHILD : function(line, argument, dataAndEvents, b, c) {
          /** @type {boolean} */
          var simple = "nth" !== line.slice(0, 3);
          /** @type {boolean} */
          var forward = "last" !== line.slice(-4);
          /** @type {boolean} */
          var ofType = "of-type" === argument;
          return 1 === b && 0 === c ? function(contestant) {
            return!!contestant.parentNode;
          } : function(node, deepDataAndEvents, dataAndEvents) {
            ...
            var name = ofType && node.nodeName.toLowerCase();
            /** @type {boolean} */
            ...
            if (parent) {
              if (simple) {
                for (;dir;) {
                  ...
                  elem = node;
                  for (;elem = elem[dir];) {
                    if (ofType ? elem.nodeName.toLowerCase() === name : 1 === elem.nodeType) {
                      return false;
                    }
                  }
                  /** @type {(boolean|string)} */
                  start = dir = "only" === line && (!start && "nextSibling");
                }
                return true;
              }
              if (start = [forward ? parent.firstChild : parent.lastChild], forward && useCache) {
                a = (nodeIndex = (ADDON_SUBTYPES_APPLEVEL = (_base = (callbacks = (elem = parent)[expando] || (elem[expando] = {}))[elem.uniqueID] || (callbacks[elem.uniqueID] = {}))[line] || [])[0] === type && ADDON_SUBTYPES_APPLEVEL[1]) && ADDON_SUBTYPES_APPLEVEL[2];
                elem = nodeIndex && parent.childNodes[nodeIndex];
                for (;elem = ++nodeIndex && (elem && elem[dir]) || ((a = nodeIndex = 0) || start.pop());) {
                  if (1 === elem.nodeType && (++a && elem === node)) {
                    /** @type {Array} */
                    _base[line] = [type, nodeIndex, a];
                    break;
                  }
                }
              } else {
                if (useCache && (a = nodeIndex = (ADDON_SUBTYPES_APPLEVEL = (_base = (callbacks = (elem = node)[expando] || (elem[expando] = {}))[elem.uniqueID] || (callbacks[elem.uniqueID] = {}))[line] || [])[0] === type && ADDON_SUBTYPES_APPLEVEL[1]), false === a) {
                  for (;elem = ++nodeIndex && (elem && elem[dir]) || ((a = nodeIndex = 0) || start.pop());) {
                    if ((ofType ? elem.nodeName.toLowerCase() === name : 1 === elem.nodeType) && (++a && (useCache && ((_base = (callbacks = elem[expando] || (elem[expando] = {}))[elem.uniqueID] || (callbacks[elem.uniqueID] = {}))[line] = [type, a]), elem === node))) {
                      break;
                    }
                  }
                }
              }
              return(a -= c) === b || a % b == 0 && a / b >= 0;
            }
          };
        },
        /**
        ...
        PSEUDO : function(pseudo, context) {
          ...
          return fn[expando] ? fn(context) : fn.length > 1 ? (args = [pseudo, pseudo, "", context], Expr.setFilters.hasOwnProperty(pseudo.toLowerCase()) ? markFunction(function(el, obj) {
            var index;
            var value = fn(el, context);
            var k = value.length;
            for (;k--;) {
              /** @type {boolean} */
              el[index = getDistance(el, value[k])] = !(obj[index] = value[k]);
            }
          }) : function(err) {
      ...
      pseudos : {
        ...
        target : function(path) {
          ...
          return models && models.slice(1) === path.id;
        },
        /**
    ...
    }, support.sortStable = expando.split("").sort(sortOrder).join("") === expando, support.detectDuplicates = !!f, setDocument(), support.sortDetached = assert(function(a) {
  ...
  (jQuery.fn.init = function(selector, context, rootjQuery) {
    ...
    return selector.nodeType ? (this[0] = selector, this.length = 1, this) : isArray(selector) ? void 0 !== rootjQuery.ready ? rootjQuery.ready(selector) : selector(jQuery) : jQuery.makeArray(selector, this);
  }).prototype = jQuery.fn;
  object = jQuery(doc);
  /** @type {RegExp} */
  ...
  jQuery.each({
    ...
    jQuery.fn[name] = function(until, selector) {
      ...
      return "Until" !== name.slice(-5) && (selector = until), selector && ("string" == typeof selector && (matched = jQuery.filter(selector, matched))), this.length > 1 && (guaranteedUnique[name] || jQuery.uniqueSort(matched), eventSplitter.test(name) && matched.reverse()), this.pushStack(matched);
    };
  });
  /** @type {RegExp} */
  ...
  jQuery.extend({
      ...
      var promise = {
        ...
        pipe : function() {
          ...
          return jQuery.Deferred(function(newDefer) {
            jQuery.each(which, function(dataAndEvents, tuple) {
              var fn = isArray(args[tuple[4]]) && args[tuple[4]];
              deferred[tuple[1]](function() {
                var returned = fn && fn.apply(this, arguments);
                if (returned && isArray(returned.promise)) {
        ...
        then : function(value, success, v) {
          ...
          function fn(s, item, first, name) {
            return function() {
              ...
              var error = function() {
                ...
                var val;
                if (!(s < e)) {
                  if ((a = first.apply(obj, args)) === item.promise()) {
                    throw new TypeError("Thenable self-resolution");
                  }
                  val = a && (("object" == typeof a || "function" == typeof a) && a.then);
                  if (isArray(val)) {
                    if (name) {
                      val.call(a, fn(e, item, index, name), fn(e, item, data, name));
                    } else {
                      e++;
                      val.call(a, fn(e, item, index, name), fn(e, item, data, name), fn(e, item, index, item.notifyWith));
                    }
                  } else {
                    if (first !== index) {
                      obj = void 0;
                    ...
                    (name || item.resolveWith)(obj, args);
                  }
                }
              };
              /** @type {function (): undefined} */
          ...
          return jQuery.Deferred(function(i) {
            which[0][3].add(fn(0, i, isArray(v) ? v : index, i.notifyWith));
            which[1][3].add(fn(0, i, isArray(value) ? value : index));
            which[2][3].add(fn(0, i, isArray(success) ? success : data));
          }).promise();
        },
        /**
         * @param {string} obj
    ...
    when : function(path) {
      ...
      if (len <= 1 && (require(path, d.done(updateFunc(i)).resolve, d.reject, !len), "pending" === d.state() || isArray(args[i] && args[i].then))) {
        return d.then();
      }
      for (;i--;) {
  ...
  var access = function(elems, fn, key, value, chainable, emptyGet, raw) {
    ...
    if ("object" === type(key)) {
      ...
      if (void 0 !== value && (chainable = true, isArray(value) || (raw = true), bulk && (raw ? (fn.call(elems, value), fn = null) : (bulk = fn, fn = function(elem, event, value) {
  ...
  Data.prototype = {
    ...
    set : function(owner, data, value) {
      ...
      if ("string" == typeof data) {
        /** @type {string} */
        cache[trim(data)] = value;
      } else {
        for (prop in data) {
          cache[trim(prop)] = data[prop];
        }
      }
      return cache;
    },
    ...
    remove : function(owner, source) {
      ...
      if (void 0 !== cache) {
        if (void 0 !== source) {
          i = (source = Array.isArray(source) ? source.map(trim) : (source = trim(source)) in cache ? [source] : source.match(core_rnotwhite) || []).length;
          for (;i--;) {
  ...
  var isHidden = function(node, context) {
    return "none" === (node = context || node).style.display || "" === node.style.display && (jQuery.contains(node.ownerDocument, node) && "none" === jQuery.css(node, "display"));
  };
  /**
   * @param {Object} elem
   * @param {Object} options
  ...
  var rhtml = /<|&#?\w+;/;
  !function() {
    var form = doc.createDocumentFragment().appendChild(doc.createElement("div"));
    var input = doc.createElement("input");
  ...
  jQuery.event = {
     ...
     * @return {undefined}
     */
    addProp : function(prop, deps) {
      Object.defineProperty(jQuery.Event.prototype, prop, {
        enumerable : true,
        configurable : true,
        /** @type {function (): ?} */
        get : isArray(deps) ? function() {
          if (this.originalEvent) {
            return deps(this.originalEvent);
          }
        } : function() {
          if (this.originalEvent) {
  ...
  !function() {
    ...
    function init() {
      if (div) {
        ...
        var ret = win.getComputedStyle(div);
        /** @type {boolean} */
        i = "1%" !== ret.top;
        /** @type {boolean} */
        u = 12 === setSize(ret.marginLeft);
        /** @type {string} */
        div.style.right = "60%";
        /** @type {boolean} */
        s = 36 === setSize(ret.right);
        /** @type {boolean} */
        o = 36 === setSize(ret.width);
        /** @type {string} */
        div.style.position = "absolute";
        /** @type {(boolean|string)} */
        absolute = 36 === div.offsetWidth || "absolute";
  ...
  jQuery.extend({
     ...
     * @return {?}
     */
    style : function(elem, name, val, value) {
      if (elem && (3 !== elem.nodeType && (8 !== elem.nodeType && elem.style))) {
        ...
        var prop = trim(name);
        /** @type {boolean} */
        var quoteNeeded = rparentsprev.test(name);
        var style = elem.style;
        if (quoteNeeded || (name = property(prop)), hooks = jQuery.cssHooks[name] || jQuery.cssHooks[prop], void 0 === val) {
          return hooks && ("get" in hooks && void 0 !== (parts = hooks.get(elem, false, value))) ? parts : style[name];
        }
        if ("string" == (current = typeof val)) {
          if (parts = rfxnum.exec(val)) {
            if (parts[1]) {
              val = get(elem, name, parts);
              /** @type {string} */
              current = "number";
            }
          }
        ...
        if (null != val) {
          if (val === val) {
            if ("number" === current) {
              val += parts && parts[3] || (jQuery.cssNumber[prop] ? "" : "px");
            }
            if (!support.clearCloneStyle) {
              if (!("" !== val)) {
                if (!(0 !== name.indexOf("background"))) {
                  /** @type {string} */
                  style[name] = "inherit";
                }
              }
  ...
  jQuery.each(["height", "width"], function(dataAndEvents, prop) {
    jQuery.cssHooks[prop] = {
      ...
      get : function(elem, value, extra) {
        if (value) {
          return!rdisplayswap.test(jQuery.css(elem, "display")) || elem.getClientRects().length && elem.getBoundingClientRect().width ? getWidthOrHeight(elem, prop, extra) : swap(elem, cssShow, function() {
      ...
      set : function(elem, value, actual) {
        ...
        /** @type {boolean} */
        var camelKey = "border-box" === jQuery.css(elem, "boxSizing", false, styles);
        var act = actual && fn(elem, prop, actual, camelKey, styles);
        return camelKey && (support.scrollboxSize() === styles.position && (act -= Math.ceil(elem["offset" + prop[0].toUpperCase() + prop.slice(1)] - parseFloat(styles[prop]) - fn(elem, prop, "border", false, styles) - 0.5))), act && ((match = rfxnum.exec(value)) && ("px" !== (match[3] || "px") && (elem.style[prop] = value, value = jQuery.css(elem, prop)))), cb(elem, value, act);
      }
    };
  });
  ...
  jQuery.Animation = jQuery.extend(Animation, {
    tweeners : {
      "*" : [function(prop, value) {
        ...
        return get(tween.elem, prop, rfxnum.exec(value), tween), tween;
      }]
    },
    /**
     * @param {Object} value
  ...
  jQuery.speed = function(speed, easing, fn) {
    var opt = speed && "object" == typeof speed ? jQuery.extend({}, speed) : {
      complete : fn || (!fn && easing || isArray(speed) && speed),
      duration : speed,
      easing : fn && easing || easing && (!isArray(easing) && easing)
    };
  ...
  (function() {
    var input = doc.createElement("input");
    var opt = doc.createElement("select").appendChild(doc.createElement("option"));
    /** @type {string} */
    input.type = "checkbox";
    /** @type {boolean} */
    ...
    /** @type {string} */
    (input = doc.createElement("input")).value = "t";
    /** @type {string} */
    input.type = "radio";
    /** @type {boolean} */
  ...
  jQuery.fn.extend({
    ...
    removeClass : function(value) {
      ...
      if ((res = find(value)).length) {
        for (;src = this[u++];) {
          if (pdataOld = each(src), cur = 1 === src.nodeType && " " + indexOf(pdataOld) + " ") {
            /** @type {number} */
            resLength = 0;
            for (;apn = res[resLength++];) {
              for (;cur.indexOf(" " + apn + " ") > -1;) {
                /** @type {string} */
                cur = cur.replace(" " + apn + " ", " ");
              }
            }
    ...
    toggleClass : function(value, stateVal) {
      ...
      return "boolean" == typeof stateVal && boolean ? stateVal ? this.addClass(value) : this.removeClass(value) : isArray(value) ? this.each(function(i) {
    ...
    hasClass : function(name) {
      ...
      tval = " " + name + " ";
      for (;which = this[r++];) {
        if (1 === which.nodeType && (" " + indexOf(each(which)) + " ").indexOf(tval) > -1) {
          return true;
        }
      }
      return false;
  ...
  jQuery.fn.extend({
    ...
    val : function(value) {
      ...
      var elem = this[0];
      if (arguments.length) {
        return isFunction = isArray(value), this.each(function(i) {
          var val;
          if (1 === this.nodeType) {
      ...
      if (elem) {
        return(hooks = jQuery.valHooks[elem.type] || jQuery.valHooks[elem.nodeName.toLowerCase()]) && ("get" in hooks && void 0 !== (ret = hooks.get(elem, "value"))) ? ret : "string" == typeof(ret = elem.value) ? ret.replace(rclass, "") : null == ret ? "" : ret;
      }
    }
  });
  jQuery.extend({
    valHooks : {
      ...
      select : {
        ...
        set : function(elem, value) {
          ...
          for (;i--;) {
            if ((cur = options[i]).selected = jQuery.inArray(jQuery.valHooks.option.get(cur), values) > -1) {
              /** @type {boolean} */
              n = true;
            }
          }
  ...
  jQuery.extend(jQuery.event, {
    ...
    trigger : function(event, data, elem, onlyHandlers) {
      ...
      if (cur = el = tmp = elem = elem || doc, 3 !== elem.nodeType && (8 !== elem.nodeType && (!rfocusMorph.test(type + jQuery.event.triggered) && (type.indexOf(".") > -1 && (type = (namespaces = type.split(".")).shift(), namespaces.sort()), ontype = type.indexOf(":") < 0 && "on" + type, event = event[jQuery.expando] ? event : new jQuery.Event(type, "object" == typeof event && event), event.isTrigger = onlyHandlers ? 2 : 3, event.namespace = namespaces.join("."), event.rnamespace = event.namespace ? 
      new RegExp("(^|\\.)" + namespaces.join("\\.(?:.*\\.|)") + "(\\.|$)") : null, event.result = void 0, event.target || (event.target = elem), data = null == data ? [event] : jQuery.makeArray(data, [event]), special = jQuery.event.special[type] || {}, onlyHandlers || (!special.trigger || false !== special.trigger.apply(elem, data)))))) {
  ...
  jQuery.param = function(a, traditional) {
    ...
    var add = function(key, value) {
      var val = isArray(value) ? value() : value;
      /** @type {string} */
  ...
  jQuery.extend({
    ...
    ajax : function(arg, opts) {
      ...
      if (ajaxSend = jQuery.event && options.global) {
        if (0 == jQuery.active++) {
      ...
      if (options.data && (options.hasContent && false !== options.contentType) || opts.contentType) {
        jqXHR.setRequestHeader("Content-Type", options.contentType);
      }
      jqXHR.setRequestHeader("Accept", options.dataTypes[0] && options.accepts[options.dataTypes[0]] ? options.accepts[options.dataTypes[0]] + ("*" !== options.dataTypes[0] ? ", " + $t + "; q=0.01" : "") : options.accepts["*"]);
  ...
  jQuery.each(["get", "post"], function(dataAndEvents, method) {
     ...
     * @return {?}
     */
    jQuery[method] = function(value, tasks, success, dataType) {
      return isArray(tasks) && (dataType = dataType || success, success = tasks, tasks = void 0), jQuery.ajax(jQuery.extend({
        url : value,
        type : method,
        dataType : dataType,
        data : tasks,
  ...
  jQuery.fn.extend({
    ...
    wrapAll : function(html) {
      var wrap;
      return this[0] && (isArray(html) && (html = html.call(this[0])), wrap = jQuery(html, this[0].ownerDocument).eq(0).clone(true), this[0].parentNode && wrap.insertBefore(this[0]), wrap.map(function() {
        var elem = this;
    ...
    wrapInner : function(html) {
      return isArray(html) ? this.each(function(i) {
   ...
   * @return {?}
   */
  jQuery.expr.pseudos.hidden = function(val) {
    return!jQuery.expr.pseudos.visible(val);
  };
  /**
   * @param {Element} node
   * @return {?}
  ...
  jQuery.ajaxPrefilter("json jsonp", function(s, originalSettings, jqXHR) {
    ...
    if (jsonProp || "jsonp" === s.dataTypes[0]) {
      return name = s.jsonpCallback = isArray(s.jsonpCallback) ? s.jsonpCallback() : s.jsonpCallback, jsonProp ? s[jsonProp] = s[jsonProp].replace(rjsonp, "$1" + name) : false !== s.jsonp && (s.url += (rquery.test(s.url) ? "&" : "?") + s.jsonp + "=" + name), s.converters["script json"] = function() {
  ...
  jQuery.parseHTML = function(data, context, keepScripts) {
    ...
    var parsed;
    var scripts;
    return context || (support.createHTMLDocument ? ((baseTag = (context = doc.implementation.createHTMLDocument("")).createElement("base")).href = doc.location.href, context.head.appendChild(baseTag)) : context = doc), parsed = rsingleTag.exec(data), scripts = !keepScripts && [], parsed ? [context.createElement(parsed[1])] : (parsed = parse([data], context, scripts), scripts && (scripts.length && jQuery(scripts).remove()), jQuery.merge([], parsed.childNodes));
  };
  /**
  ...
  jQuery.fn.load = function(source, data, callback) {
    ...
    return n > -1 && (selector = indexOf(source.slice(n)), source = source.slice(0, n)), isArray(data) ? (callback = data, data = void 0) : data && ("object" == typeof data && (method = "POST")), self.length > 0 && jQuery.ajax({
      url : source,
      type : method || "GET",
      dataType : "html",
      data : data
  ...
  jQuery.offset = {
    ...
    setOffset : function(elem, options, i) {
      ...
      if (l = ("absolute" === mode || "fixed" === mode) && (curCSSTop + curCSSLeft).indexOf("auto") > -1) {
  ...
  jQuery.fn.extend({
    ...
    offset : function(options) {
      ...
      var node = this[0];
      if (node) {
        return node.getClientRects().length ? (box = node.getBoundingClientRect(), win = node.ownerDocument.defaultView, {
  ...
  jQuery.proxy = function(fn, context) {
    ...
    var args;
    var proxy;
    if ("string" == typeof context && (tmp = fn[context], context = fn, fn = tmp), isArray(fn)) {
